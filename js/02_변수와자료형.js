...
function varCheck(){
  ...
  // var를 이용할 경우 변수명이 중복되면 이전 값을 새로운 값으로 덮어쓰기함
  // 문제점 : 이전에 대입한 값을 이용할 수 없다
  // -> 나중에 이전 값을 사용하고 싶은데, 나도 모르게 새로운 값을 대입해 지워버리는 경우
...
/* 3. const */
/* 상수(consteant, 항상 같은 수), 상수명 중복 X, 블록 {} 레벨 범위 */
// const로 선언된 변수에 값을 대입하면 변경 할 수 없다 (새로운 값 대입 불가)
function constCheck(){
  ...
  // 02_변수와자료형.js:87 -> 87번째 줄에서 오류 발생
...
/* 선수 지식
  ...
  2. 지역 변수 : 특정 지역에서 사용 가능한 변수
    1) 함수 레벨 범위
      함수 정의 부분에서 {} 내부에서 var로 선언된 변수는 함수 내부 어디서든 사용가능
...
function scopeTest2(){
  ...
  if(1 == 1){ // 1과 1이 같다면 if {} 수행
    ...
    console.log("if 내부에서 global :", global1); // 출력 O
...
function typeCheck(){
  ...
  console.log("name :", phone, typeof phone);
  ...
  console.log("age :", sight, typeof sight);
